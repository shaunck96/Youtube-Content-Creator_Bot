# -*- coding: utf-8 -*-
"""Content_Creator_Assistance.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1buzh_jLdLFdztfjLnleewzmzWFJKVrlc
"""
...
#yt.extract_all()
#channel_stats = yt.to_dataframe()
#channel_stats.to_csv(r'/content/JoeRogan_channel_stats.csv')
#channel_stats['duration_minutes'] = channel_stats['duration'].apply(extract_minutes)
#less_than_5 = channel_stats[channel_stats['duration_minutes']<8]
...
# Now, url_list contains all the URLs from the file
print(url_list)
...
url_mp3_mapping = url_transcriber(url_list[:30],'/content/sniper_wof_23Nov')
url_mp3_mapping

#USING CHANNEL STAT DATA
#top_ten_url_list = []
#for i in list(less_than_5.sort_values(by=['viewCount'], ascending=False)['video_id']):
#  top_ten_url_list.append("https://www.youtube.com/watch?v="+str(i))

#url_mp3_mapping = url_transcriber(top_ten_url_list,'/content/academy_of_wealth/top_ten_viewed_less_than_5minutes')
#url_mp3_mapping
...
chat_model = ChatOpenAI(model_name="gpt-4", temperature=0.5, openai_api_key = "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")
...
for transcription in result.split('\n\n'):
  if num_tokens_from_string(transcription, "gpt-3.5-turbo-16k")<16000:
...
sentiment_output_custom_functions = [
    {
        ...
        'parameters': {
            ...
            'properties': {
                'positive_sentiment': {
                    'type': 'string',
                    'description': '{{comment 1: topic, comment 2: topic, ...}}'
                },
                'negative_sentiment': {
                    'type': 'string',
                    'description': '{{comment 1: topic, comment 2: topic, ...}}'
...
for url,comment_chunk in urls_and_comments.items():
  if len(comment_chunk) == 0:
    print("Failed")
  else:
    comment = str(comment_chunk)
    sentiment_topic_scraper = [
        {
            'name': 'extract_sentiment_topics',
            'description': 'Get high level topics driving positive and negative sentiments towards the video from the json object',
            'parameters': {
                'type': 'object',
                'properties': {
                    'positive_sentiment': {
    ...
    sentiment_topic_extraction_prompt = f'''
    Please extract the high level topics from the given group of comments delimited by \n indicating positive and negative sentiment. Return it as a JSON object:

    positive sentiment - topic1, topic2, ...
    negative sentiment - topic1, topic2, ...

    This is the json object to extract the information from:
    {comment_chunk}
    '''

    response = openai.ChatCompletion.create(
        model = 'gpt-3.5-turbo',
        messages = [{'role': 'user', 'content': str(comment_chunk)}],
        functions = sentiment_topic_scraper,
        function_call = 'auto'
    )
    # Loading the response as a JSON object
    json_response_two = json.loads(response['choices'][0]['message']['function_call']['arguments'])
    print(json_response_two)

    pos_neg_sentiment_topics[url] = json_response_two

pos_neg_sentiment_topics

pos_neg_sentiment_topics.keys()

topic_dict

sentiment_df = pd.DataFrame(columns=['video_id','url','sentiment_topics'])
sentiment_df['url'] = list(pos_neg_sentiment_topics.keys())
sentiment_df['video_id'] = sentiment_df['url'].apply(lambda x: video_id_from_url(x))
sentiment_df['sentiment_topics'] = list(pos_neg_sentiment_topics.values())

topic_df = pd.DataFrame(columns=['video_id','topics'])
topic_df['video_id'] = list(list(topic_dict.keys()))
topic_df['topics'] = list(list(topic_dict.values()))

eval_df = topic_df.merge(sentiment_df, on=['video_id'], how='inner')
eval_df
...
"""#Using Key Word to search videos, links ..
from apiclient.discovery import build

# Arguments that need to passed to the build function
DEVELOPER_KEY = "AIzaSyA-8FLWOb8Pk9ovjhHIbykqlTbTnXpxZms"
